...
module.exports = new CommandBuilder()
    ...
    .setExecute(async (message, user, args) => {
        ...
        const words = await (await wordDB.all()).filter(v => v.key.slice(0, message.guild.id.length || message.server.id.length) === (message.guild.id || message.server.id));
        for (const word in words) {
            await message.channel.send(`||${words[word].key.slice((message.guild.id.length || message.server.id.length), words[word].key.length)}||`)
        }
    })